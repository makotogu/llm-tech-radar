...
| 证据 | 支撑的判断 | 边界 |
| --- | --- | --- |
| [OpenAI Prompt engineering](https://developers.openai.com/api/docs/guides/prompt-engineering) | 指令、上下文、示例和输出要求需要清晰分层 | 官方建议会随模型变化 |
| [GitHub Copilot 提效实验](../../sources/cards/github-copilot-productivity.md) | Prompt 或助手试点需要固定任务、对照和质量指标 | 受控任务不能外推到所有研发流程 |
| [DSPy](../../sources/cards/dspy-pipeline-optimization.md) | Prompt 优化应连接数据集和 metric，而不是只靠人工试词 | 自动优化依赖正确的目标函数 |

当前判断：Prompt 是可版本化的任务接口，不是独立于 Context、工具和评测存在的“文案技巧”。稳定性只能通过固定样例和失败分类证明。当前已有 [Prompt Review Workflow Lab](../../labs/prompt-review-workflow/README.md)；下一步缺口是把安全拒答和多语言样例加入同一回归集。
